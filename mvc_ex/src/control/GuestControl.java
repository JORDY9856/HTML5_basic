...
public class GuestControl extends HttpServlet {
	...
	private void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( cmdKey == null ){
			cmdKey = "main-page"; //list-page 들어옴
		}
		...
		try{
			...
			if( commandMap.containsKey( cmdKey ) ){ //해시맵에 이 값이 있니?
				cmd = (Command)commandMap.get( cmdKey);
			}else{
				throw new CommandException("지정할 명령어가 존재하지 않음");
			}

			nextPage = cmd.execute( request, response  ); //호출할께
		...
		reqDp.forward( request, response ); //db를 포워딩해서 출력
